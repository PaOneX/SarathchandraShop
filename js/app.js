...
fetch("json/newArrivals.json")
    ...
    .then((data) => {
        ...
        data.forEach((item) => {
            ...
            let badgeHTML = '';
            if (item.badge) {
                const badgeClass = item.badge.type === 'danger' ? 'bg-danger' : 
                                  item.badge.type === 'primary' ? 'bg-success' : 
                                  'bg-warning';
                badgeHTML = `<span class="badge ${badgeClass} product-badge rounded-pill">${item.badge.text}</span>`;
            }

            let priceHTML = `<span class="h4 mb-0 fw-bold">$${item.price.current.toFixed(2)}</span>`;
            if (item.price.old) {
                priceHTML += `<span class="text-muted text-decoration-line-through">$${item.price.old.toFixed(2)}</span>`;
            }
